...
const highlightMatch = (text, query) => {
    ...
    return text.split(regex).map((part, i) =>
        regex.test(part)
            ? <mark key={i} className="bg-yellow-200 dark:bg-yellow-600 rounded">{part}</mark>
            : part
    );
};

const CountryCard = React.memo(function CountryCard({ country, search = '' }) {
        ...
        >
            ...
            <CardContent className="pt-4 pb-6 px-4">
                ...
                <p className="text-sm text-muted-foreground space-y-1">
                    ...
                    <span className="block"><strong>Capital:</strong> {country.capital?.[0] || 'N/A'}</span>
                    <span className="block"><strong>Population:</strong> {populationText}</span>
                </p>
            </CardContent>
        </Card>
    );
